...
"""
    ...
    Purpose: 
        ...
        In this edition we use MongoDB to store numbers or strings we generate
    
    Introduction:
        A class for generate some random numbers or strings
        
    Special Tips:
        This class was made for using in Random_mongo_gener.py
...
class Random_gener(object):
    ...
    def gener(self, datatype, datarange, num, strlen=10):
        ...
        Introduction
        ------------
        Core part of generate the items you need, int float and str are supported
        ...
        try:
            ...
            ans = 0 #to record how many items we have generated
            if datatype is float:
                while ans<num:
                ...
                while ans<num:
                    yield int(random.random()*(datarange[1]-datarange[0])+datarange[0]) #yield them!
                    ans += 1
            elif datatype is str:
                while ans<num:
                    yield ''.join(random.sample(datarange,strlen)) #yield them!
                    ans += 1
