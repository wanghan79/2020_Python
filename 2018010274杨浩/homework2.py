...
#decorator
def rand_generate(func):
        def wrapper(elem, cond):
            try:
            ...
            print('given set is', elem.collection)
            return func(elem, cond)
        return wrapper


def apply(data_type, num, domain, condition, str_len=8):
    elem = Elem(data_type, num, domain, str_len=8)
    @rand_generate
    def screening(elem, cond):
        try:
            ...
            return result
    r = screening(elem, condition)
    print('screened set is', r)
#test case 1: #test case 1: 选出包含s的字符串
apply(str, 100, string.ascii_letters, 's')
#test case 2: 选出10到20之间的数
apply(int, 100, [0, 100], [10, 20])
#test case 3: 选出0到1之间的浮点数
apply(float, 100, [0, 10], [0, 1])
