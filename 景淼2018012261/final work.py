"""
Author:JingMiao
Purpose:Generate random data set.MongoDB
Create:30/6/2020
"""
import random
import string
...
collist=mydb.list_collection.names()
if "sites" in collist:
    print("集合已存在！")
...
def dataScreening(*args):
    try:

        result = set()

        for index in mycol.find({}, {"datatype": "int" or "float"}):
            it = iter(args)
            if next(it) <= index <= next(it):
                result.add(index)

        for index in mycol.find({}, {"datatype": "str"}):
             for it in args:
                   if it in index:
                        result.add(index)
        return result
...
def apply():
    ...
    for index in result1:
        mydict={"datatype":"int","datarange":"1-10","num":"3","data":"index"}
    ...
    for index in result2:
        mydict={"datatype":"float","datarange":"-2.0-10.0","num":"10","data":"index"}
        x=mycol.insert_one(mydict)
    ...
    for index in result3:
        mydict={"datatype":"str","datarange":"string.ascii_letters+string.digits","num":"5","strlen":"2","data":"index"}
        x=mycol.insert_one(mydict)
