...
# 使用MongoDB存储平时作业3中生成的随机数，并从MongoDB中查询数据和进行数据筛选。
myClient = pymongo.MongoClient("mongodb://localhost:27017/")
mydb = myClient["randomDataDB"]
dblist = myClient.list_database_names()
if "randomDataDB" in dblist:
    print('随机数据集存在！')
myCol_int = mydb["intData"]
myCol_float = mydb["floatData"]
myCol_str = mydb["strData"]
col_list = mydb.list_collection_names()
if "intData" in col_list:
    print("intData exists!")
    myCol_int.delete_many({})
if "floatData" in col_list:
    print("floatData exists!")
    myCol_float.delete_many({})
if "strData" in col_list:
    print("strData exists!")
    myCol_str.delete_many({})
